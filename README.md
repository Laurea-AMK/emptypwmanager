...
2.  You should implement the following functions: adding password, restrieving password,  loading passwords from file and saving passwords to file. Optionionally, add a password strengh checker and random password generator.
3.  Please use the provided funtions to encrypt and decrypt the password instead of writing them to file with plain text (while noting this encryption methos is only for educational purposes and not strong enough for real password managers).
4.  Follow instructions in the coding templete on how to implement the functions.  If you need to devert from the instructions, explain clearly in commends why you have done so.
